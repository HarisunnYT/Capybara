...
void ProceduralSampleSimple_half (
...
)
{
    ...
    float w1, w2, w3; // Changed to float
    ...
    const float2x2 hashMatrix = float2x2(127.1f, 311.7f, 269.5f, 183.3f);
    const float hashFactor = 3758.5453f;
...
//  Get weights
    float exponent = 1.0f + Blend * 15.0f; // Changed to float
    w1 = pow(w1, exponent);
    w2 = pow(w2, exponent);
    w3 = pow(w3, exponent);

//  Lets help the compiler here:
    float sum = 1.0f / (w1 + w2 + w3); // Changed to float
    w1 = w1 * sum;
    w2 = w2 * sum;
    w3 = w3 * sum;
...
void ProceduralSampleSimple_float(
    SamplerState samplerTex,
//  Base inputs
    half Blend,
    float2 uv,
